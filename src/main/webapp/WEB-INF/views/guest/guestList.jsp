  ...
  <table class="table table-borderless m-0 p-0">
  	...
  	<tr>
  	  <td colspan="2" class="text-end">
  	    <c:if test="${pag > 1}">
  	      <a href="guestList?pag=1" title="첫페이지" class="text-decoration-none btn btn-secondary btn-sm">◁◁</a>
  	      <a href="guestList?pag=${pag-1}" title="이전페이지" class="text-decoration-none btn btn-secondary btn-sm">◀</a>
  	    </c:if>
  	    ${pag}  / ${totPage}
  	    <c:if test="${pag < totPage}">
  	      <a href="guestList?pag=${pag+1}" title="다음페이지" class="text-decoration-none btn btn-secondary btn-sm">▶</a>
  	      <a href="guestList?pag=${totPage}" title="마지막페이지" class="text-decoration-none btn btn-secondary btn-sm">▷▷</a>
  	    </c:if>
  	  </td>
  	</tr>
  </table>
...
<div class="text-center">
  ...
  <c:if test="${curBlock < lastBlock}"><a href="guestList?pag=${(curBlock+1)*blockSize+1}" class="text-decoration-none btn btn-secondary btn-sm">다음블록</a></c:if>
  <c:if test="${pag < totPage}"><a href="guestList?pag=${totPage}" class="text-decoration-none btn btn-secondary btn-sm">마지막페이지</a></c:if>
</div>
<!-- 블록페이지 끝 -->
