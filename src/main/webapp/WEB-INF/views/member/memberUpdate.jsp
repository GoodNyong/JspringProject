		...
		<form name="myform" method="post" enctype="multipart/form-data">
			<table class="table table-bordered text-center border-secondary-subtle">
				...
				<tr>
					...
					<td>
						<div class="input-group">
							<input type="text" name="nickName" id="nickName" value="${sNickName}" required class="form-control" /> <input type="button" value="닉네임중복체크" id="nickNameBtn" onclick="nickCheck()" class="btn btn-secondary btn-sm" />
						</div>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input type="radio" name="gender" id="gender1" value="남자" ${vo.gender=='남자' ? 'checked' : ''} />남자 &nbsp; <input type="radio" name="gender" id="gender2" value="여자" ${vo.gender=='여자' ? 'checked' : ''} />여자</td>
				</tr>
				<tr>
					...
					<td>
						<div class="input-group">
							...
							<input type="text" name="email1" id="email1" value="${email[0]}" class="form-control" required />@ <select name="email2" id="email2" class="form-select">
			...
			<!-- 선택입력사항 -->
			<div class="mb-2" id="viewCheckBtn" onclick="viewBtnCheck()">
				<input type="checkbox" name="viewBtn" id="viewBtn" checked />선택입력항목 보이기/가리기
			</div>
			<div id="addContent">
				<table class="table table-bordered text-center border-secondary-subtle">
					<tr>
					...
					<tr>
						...
						<td>
							<div class="input-group">
								...
								</select>- <input type="text" name="tel2" id="tel2" value="${fn:trim(tel[1])}" class="form-control" />- <input type="text" name="tel3" id="tel3" value="${fn:trim(tel[2])}" class="form-control" />
							</div>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div class="form-group">
								<div class="input-group">
									...
									<input type="text" name="postcode" id="sample6_postcode" value="${fn:trim(address[0])}" class="form-control"> <input type="button" onclick="sample6_execDaumPostcode()" value="우편번호 찾기" class="btn btn-info">
								</div>
								<input type="text" name="roadAddress" id="sample6_address" value="${fn:trim(address[1])}" class="form-control">
								<div class="input-group">
									<input type="text" name="detailAddress" id="sample6_detailAddress" value="${fn:trim(address[2])}" class="form-control"> <input type="text" name="extraAddress" id="sample6_extraAddress" value="${fn:trim(address[3])}" class="form-control">
								</div>
							</div>
						</td>
					</tr>
					...
					<tr>
						...
						<td class="text-start"><c:set var="varHobbys" value="${fn:split('등산/낚시/바둑/수영/독서/승마/영화감상/축구/농구/배구/기타','/')}" /> <c:forEach var="tempHobby" items="${varHobbys}" varStatus="st">
					...
					<tr>
						...
						<td><textarea rows="5" name="content" class="form-control" placeholder="자기소개를 입력하세요.">${vo.content}</textarea></td>
					</tr>
					<tr>
						<th class="bg-secondary-subtle">정보공개</th>
						<td class="text-start"><input type="radio" name="userInfor" id="userinfor1" value="공개" ${vo.userInfor=='공개'   ? 'checked' : ''} />공개 &nbsp; <input type="radio" name="userInfor" id="userinfor2" value="비공개" ${vo.userInfor=='비공개' ? 'checked' : ''} />비공개</td>
					</tr>
					<tr>
						<th class="bg-secondary-subtle">회원사진</th>
						<td class="text-start">회원사진 : <img src="${ctp}/member/${vo.photo}" width="100px" /> <input type="file" name="fName" id="file" class="form-control" />
						</td>
					</tr>
				</table>
			</div>
			<!-- <div class="text-center" id="addBtn" style="display:none"> -->
			<input type="button" value="회원정보수정" onclick="fCheck()" class="btn btn-success me-2" /> <input type="reset" value="다시입력" class="btn btn-warning me-2" /> <input type="button" value="돌아가기" onclick="location.href='${ctp}/member/memberMain';" class="btn btn-primary" />
			<!-- </div> -->
			<input type="hidden" name="email" /> <input type="hidden" name="tel" /> <input type="hidden" name="address" /> <input type="hidden" name="photo" id="photo" value="${vo.photo}" />
		</form>
	</div>
	<p>
		<br />
